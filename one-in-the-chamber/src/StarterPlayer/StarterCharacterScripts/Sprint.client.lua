-- StarterCharacterScripts Sprint Script
local player = game.Players.LocalPlayer
local character = script.Parent
local humanoid = character:WaitForChild("Humanoid")
local fx = game.ReplicatedStorage:WaitForChild("Fx")
local WALKSPEED = 12
local RUNSPEED = 24
local lastRun = 0
local direction = 1
...
local speedCtrl = SpeedController.Get(humanoid, WALKSPEED, RUNSPEED/WALKSPEED)
humanoid:SetAttribute("BlockSprint", false)
uis.InputBegan:Connect(function(input,gpe)
	if gpe then return end
	if input.KeyCode==Enum.KeyCode.ButtonL3 then
		if not humanoid:GetAttribute("BlockSprint") then
...
rs.Heartbeat:Connect(function()
	if (character.PrimaryPart.Velocity*Vector3.new(1,0,1)).Magnitude > WALKSPEED*2 and humanoid.FloorMaterial~=Enum.Material.Air then
		...
		for i=1,math.random(1,3)do
			...
			if colorRand==1 then hb.Color=Color3.fromRGB(86,86,86) else hb.Color=Color3.fromRGB(175,175,175) end
