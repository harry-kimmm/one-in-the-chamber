...
local MAP_SPAWNS    = workspace:WaitForChild("Map"):WaitForChild("SpawnPoints")
...
local function broadcastAll(evt, ...) evt:FireAllClients(...) end
...
local function teleportTo(player, spawns)
	...
	if root then
		...
		root.CFrame = pts[math.random(1,#pts)].CFrame + Vector3.new(0,3,0)
	end
end
...
local function giveLoadout(player)
	...
	for _, key in ipairs({"EquippedRanged","EquippedMelee"}) do
		...
		if name then
			...
			if tmpl then tmpl:Clone().Parent = player.Backpack end
		end
	end
end
...
local function startRound()
	currentPhase = "Round"
	print("[Phase] Round start")
	broadcastAll(evBeginRound)
	for _, pl in ipairs(Players:GetPlayers()) do
		local ammo = pl:FindFirstChild("Ammo")
		...
		giveLoadout(pl)
		if pl.Character then teleportTo(pl, MAP_SPAWNS) end
	end
	local earlyWinner
	...
	if #Players:GetPlayers() >= MIN_PLAYERS then
		...
		if not winner then
			...
			for _, pl in ipairs(Players:GetPlayers()) do
				...
				if v>maxK then maxK, leaders = v, {pl}
				elseif v==maxK then table.insert(leaders,pl) end
			end
			if #leaders==1 then winner = leaders[1].Name end
		end
		for _, pl in ipairs(Players:GetPlayers()) do
			pl.Coins.Value += (pl.Name==winner and 100 or 50)
		end
